...
public class FlightACheck {
	...
	public static boolean check(Player p, HackData data, PlayerMoveEvent e){
		...
		if (p.getAllowFlight()) {
		    ...
		    if(p.getActivePotionEffects().contains(PotionEffectType.JUMP)){
		    	return false;
		    }
		    ...
		    if(data.flight_violations > 5){
		    	String kick = Core.getInstance().getConfig().getString("flight-kick").replace("&", "§").replaceAll("<name>", p.getName()).replaceAll("<type>", "A").replaceAll("<diff>", "" + off).replaceAll("<allowed>", "0.41");
				StaffUtil.sendStaffMessage(Core.getInstance().getConfig().getString("plugin-prefix").replaceAll("&", "§") + kick);
				String kickmsg = Core.getInstance().getConfig().getString("kick-message").replaceAll("&", "§").replaceAll("<check>", "Flight").replaceAll("<type>", "A");
				p.kickPlayer(kickmsg);
				e.setCancelled(true);
		    	return true;
		    }
		    String not = Core.getInstance().getConfig().getString("flight-notify").replace("&", "§").replaceAll("<name>", p.getName()).replaceAll("<type>", "A").replaceAll("<diff>", "" + off).replaceAll("<allowed>", "0.41");
